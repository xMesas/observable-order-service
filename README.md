...
**The polling timeout needed more headroom on CI than on a local run.** A manual `docker compose up` + `curl` run on the machine this was built on showed the trace becoming queryable in Zipkin within ~1.7 seconds of the request completing — comfortably inside the original 10-second Awaitility timeout. CI's second run still timed out at 10 seconds, never once seeing a 2xx from Zipkin. Rather than guess at GitHub Actions' exact resource constraints, the timeout was widened to 30 seconds (roughly 17x the locally-measured time) — a pragmatic margin for a slower/more contended CI runner rather than a diagnosis of the exact bottleneck.
